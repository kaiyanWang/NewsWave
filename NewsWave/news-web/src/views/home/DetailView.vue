   ...
   <el-col :span="6">
     <el-card>
       ...
       <div style="text-align:center;position:relative;bottom:45px;">
          <el-avatar :size="90" :src="BASE_URL+content.userImgUrl" style="border:5px solid #fff;"></el-avatar>
...
const loadComments = (id)=>{
  axios.get(BASE_URL+'/v1/comments/'+id).then((response)=>{
...
const post = ()=>{
  ...
  axios.post(BASE_URL+'/v1/comments/add-new',data)
      .then((response)=>{
        if(response.data.code==2001){
...
const initData = ()=>{
  ...
  //一进到当前页面就需要加载评论
  loadComments(id);
  axios.get(BASE_URL+'/v1/contents/'+id+'/detail')
      .then((response)=>{
        if(response.data.code==2001){
          content.value = response.data.data;
          //一定要在获取内容详情后再获取作者的其它文章,因为这里有用户id了
          axios.get(BASE_URL+'/v1/contents/'+content.value.userId+'/other')
              .then((response)=>{
                if(response.data.code==2001){
